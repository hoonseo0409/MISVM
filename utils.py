...
def get_points_moon(num_points, cls = 0, max_tries = 100):
    num_points = int(num_points)
    points = []
    num_points_acc = 0
    for ti in range(max_tries):
        X,  y = make_moons(n_samples= num_points * 3)
        X = X[y == cls]
        num_samples_cls = X.shape[0]
        if num_samples_cls + num_points_acc >= num_points:
            points.append(X[:num_points - num_points_acc, :])
            return np.concatenate(points, axis= 0).T
        else:
            points.append(X)
            num_points_acc += num_samples_cls
...
def convert_img_to_vector(img_arr, img_split = None, if_concat = True, model = "pftas", model_kwargs = None):
    if img_split is None: img_split = [1, 1]
    if model_kwargs is None: model_kwargs = {}
    result = []
    x_pos = []
    y_pos = []
    step_sizes = [int(img_arr.shape[0] / img_split[0]), int(img_arr.shape[1] / img_split[1])]
    for x_step in range(img_split[0]):
        for y_step in range(img_split[1]):
            ...
            x_pos.append(x_range)
            y_pos.append(y_range)
            subimg = img_arr[x_range[0] : x_range[1], y_range[0] : y_range[1]]
            if model == "pftas":
            ...
            if if_concat:
                result = result + processed_patch
            else:
                result.append(processed_patch)
    return result, x_pos, y_pos

def load_chest(chest_path="path/to/dataset", img_split = None, modality= None, model = "pftas", model_kwargs = None, min_max_scale= False, blood_features = None, static_features = None, target= None, cls = "finding"):
...
def chest_dataset_output(bags, target, image_path, x, y, M_cut_idx, bags_M, similarities, cls_bags, num_instances_fixed = None):
    ...
    if num_instances_fixed is not None:
        bags_loc = []
        for bag in bags:
            bag_loc = bag[:num_instances_fixed]
            if len(bag) < num_instances_fixed:
                for i in range(num_instances_fixed - len(bag)):
                    bag_loc.append(deepcopy(bag[-1]))
            bags_loc.append(bag_loc)
    else:
        bags_loc = bags
    bags = [np.stack(bag).T for bag in bags_loc]
...
def get_bags_with_cls_features(bags, cls):
    dummies = get_dummies(cls).to_numpy().astype(float)
    bags_with_cls_features = []
    for bi in range(len(bags)):
        bags_with_cls_features.append(np.concatenate([bags[bi], np.array([dummies[bi] for j in range(bags[bi].shape[1])]).T], axis= 0))
    return bags_with_cls_features
...
def load_synthetic_mi(class_radius, class_abnormal_threshold, num_points_factor = 1.0, num_instances_range= None):
    aux_classes = class_radius.keys()
    for dct in [class_abnormal_threshold]:
        for cls in aux_classes:
            assert(cls in dct.keys())
    if num_instances_range is None: 
        num_instances_range = (3, 7)
    elif not isinstance(num_instances_range, (list, tuple)):
        num_instances_range = (num_instances_range, num_instances_range+1)            
    ...
    for cls in aux_classes:
        ...
        if True:
            list_of_centers = [
                get_points_inside_circle(center_xy= [0., 0.], radius_max= class_abnormal_threshold[cls] + 3, num_points= 400, radius_min= class_abnormal_threshold[cls]),
                ...
                get_points_inside_circle(center_xy= [0., 0.], radius_max= class_abnormal_threshold[cls], num_points= 400, radius_min= 0),
                get_points_inside_circle(center_xy= [0., 0.], radius_max= class_abnormal_threshold[cls], num_points= 100, radius_min= 0)
                ]
        else:
            list_of_centers = [
        ...
        for normality, bool_labeled, centers_of_bags in zip([1, 1, -1, -1], [True, False, True, False], list_of_centers):
            for i in range(centers_of_bags.shape[1]):
                bags.append(get_points_inside_circle(center_xy= [centers_of_bags[0, i], centers_of_bags[1, i]], radius_max= class_radius[cls], num_points= sample(list(range(num_instances_range[0], num_instances_range[1])), 1)[0] * num_points_factor))
