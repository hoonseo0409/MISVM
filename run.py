...
base_path = "/path/to/here"
torch.set_default_device("mps")
task_type = "OC"
assert(task_type in ["OC", "PU"])
num_instances_range = (3, 7)
min_num_instances = num_instances_range[0] if isinstance(num_instances_range, (list, tuple)) else num_instances_range
...
int_of_target = 1 ## signal for inlier or normal, 1 for PU, 
models_dict = {}

if task_type == "OC": # Our model
    models_dict["ours"] = {"model": MISVM.MISVM, "data_process": ours_data_process, "init_kwargs": dict(kernel_method= "rbf", kernel_kwargs= {"sigma": 1.5}, normal_int_ratio= [-1.6, 1.6], version= 6, thres= pythresh.thresholds.filter.FILTER())}

if True: ## kenchi, fast models
...
if False: ## WrapPyod, took long time
...
if False: ## WrapDeepod, took long time
    """
    TODOs to fit deepod in MacOS.
    - Install torch through conda to make torch.set_default_device(torch.device("mps")) work.
    - Change batch to np.float32(batch) because mps does not support float 64.
    - Change device = "cuda" in deepod to "mps".
    """
    from deepod.models.tabular import NeuTraL, DeepIsolationForest, SLAD, ICL, RCA
    from deepod.models.time_series import AnomalyTransformer
    for name, kwargs in zip(["AnomalyTransformer", "RCA", "NeuTraL"], [dict(base= AnomalyTransformer, seq_len= min_num_instances), dict(base= RCA), dict(base= NeuTraL),]):
        models_dict[name] = {"model": other_models.WrapDeepod, "data_process": ours_data_process, "init_kwargs": kwargs}

if task_type == "PU" and True:
    import pulearn
...
if dataset_kind == "synthetic_mi":
    ...
    bags, cls_bags, is_normal, circles_dicts, min_max_dict, cls_bags_known = utils.load_synthetic_mi(class_radius, class_abnormal_threshold, num_points_factor = 1.0, num_instances_range= num_instances_range)
if dataset_kind == "chest-xray":
    dataset_raw =utils.load_chest()
    image_path = dataset_raw[2]
    dataset = utils.chest_dataset_output(*dataset_raw, num_instances_fixed= min_num_instances)
...
for si in range(len(splits)):
    ...
    for model_name in models_dict.keys():
        ...
        y_pred = model_inst.predict(**models_dict[model_name]["data_process"](X= [bags_loc[i] for i in split["test"]], y= [cls_bags[i] for i in split["test"]], phase= "test")) ## 1 is inlier, -1 is outlier.
        ...
        results_dict[model_name][si]["CT"] = time() - ctime
        for metric in ["precision", "recall", "f1", "accuracy", "bacc", "CT"]:
            print(f"{metric}: {results_dict[model_name][si][metric]}.")    

        if dataset_kind == "synthetic_mi" and model_name.startswith("ours"):
            for pi in range(3):
